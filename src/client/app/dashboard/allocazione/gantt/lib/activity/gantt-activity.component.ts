...
@Component({
    ...
    template: `
    <div>
        <div class="grid_scale" [ngStyle]="{'width':'auto'}">
     <!--  <input type="text" name="filter" [(ngModel)]="term" class="filter">  -->
         <div class="grid_head_cell" *ngFor="let column of gridColumns" [style.width]="column.width - 15 + 'px'" [style.left]="column.left + 'px'">
             </div>
           
        </div>
    </div>
    <div class="grid" #ganttGrid [ngStyle]="{ 'height': ganttActivityHeight, 'width':'30%'}">
    ...
    <div class="grid_data" #ganttGridData [ngStyle]="{ 'height': ganttService.calculateGanttHeight() ,'width':'auto'  }">
    <div #row *ngFor="let data of ganttService.groupData(ganttService.TASK_CACHE)|filter:term" (click)="toggleChildren(row, data)" 
    class="grid_row" [ngStyle]="setGridRowStyle(ganttService.isParent(data.treePath))" [attr.data-id]="ganttService.setIdPrefix(data.id)" 
     [attr.data-isParent]="ganttService.isParent(data.treePath)" [attr.data-parentid]="ganttService.setIdPrefix(data.parentId)" (change)="filterGantt()">
    ...
    </div><div class="gantt_activity" (window:resize)="onResize($event)" [ngStyle]="{ 'height': ganttActivityHeight, 'width': '70%'}">
...
export class GanttActivityComponent implements OnInit, DoCheck {
    ...
    public gridColumns: any[] = [
        { name: 'Risorse', left: 20, width: 200 },
        { name: 'Progetto', left: 14, width: 150 },
        { name: 'Durata', left: 20, width: 200 }
    ];
