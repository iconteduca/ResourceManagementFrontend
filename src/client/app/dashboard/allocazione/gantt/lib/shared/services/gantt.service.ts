...
export class GanttService {
    ...
    private calculateBarLeft(start: Date, scale: any[], hours?: boolean): number {
        ...
        if (start != null) {
            ...
            for (var i = 0; i < scale.length; i++) {
                if (start.getDate() === scale[i].getDate()&& start.getMonth() === scale[i].getMonth()) {
                    if (hours) {
   ...
   public calculateGanttWidth(): string {        
        return '30%';
    }
    ...
    public calculateDuration(task: any): string {
        try {
            if (task.start != null && task.end != null) {
                ...
                if (duration > 24) {
                    return `${Math.round(duration / 24)} giorno/i`; // duration in days
                } else if (duration > 1) {
